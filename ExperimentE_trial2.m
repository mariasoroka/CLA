...
for j=1:k
   ...
   % disp(mynGauss) 
    ...
    if n_Gausspd>mynGauss
        mean_log_resid_G_LS_dense =    [(1/(j+1))*(j*mean_log_resid_G_LS_dense(1:mynGauss)+log_resid_G_LS_dense(1:mynGauss)), ...
            log_resid_G_LS_dense(mynGauss:end)];
        mean_log_time_G_LS_dense  = [(1/(j+1))*(j*mean_log_time_G_LS_dense(1:mynGauss)+log_time_G_LS_dense(1:mynGauss)),...
            log_time_G_LS_dense(mynGauss:end)];
        mean_log_flops_G_LS_dense = [(1/(j+1))*(j*mean_log_flops_G_LS_dense(1:mynGauss)+log_flops_G_LS_dense(1:mynGauss)),...
            log_flops_G_LS_dense(mynGauss:end)];
    end
    if n_Gausspd<mynGauss
        mean_log_resid_G_LS_dense =    [(1/(j+1))*(j*mean_log_resid_G_LS_dense(1:n_Gausspd)+log_resid_G_LS_dense(1:n_Gausspd)), ...
            mean_log_resid_G_LS_dense(n_Gausspd+1:end)];
        mean_log_time_G_LS_dense  = [(1/(j+1))*(j*mean_log_time_G_LS_dense(1:n_Gausspd)+log_time_G_LS_dense(1:n_Gausspd)),...
            mean_log_time_G_LS_dense(n_Gausspd+1:end)];
        mean_log_flops_G_LS_dense = [(1/(j+1))*(j*mean_log_flops_G_LS_dense(1:n_Gausspd)+log_flops_G_LS_dense(1:n_Gausspd)),...
            mean_log_flops_G_LS_dense(n_Gausspd+1:end)];
    end
    [myvar,mynGauss] =size(mean_log_flops_G_LS_dense);
    ...
    mean_log_resid_CD_LS_dense = [(1/(j+1))*(j*mean_log_resid_CD_LS_dense(1:mynCD)+log_resid_CD_LS_dense(1:mynCD)),...
           mean_log_resid_CD_LS_dense(mynCD:end),log_resid_CD_LS_dense(mynCD:end)];
    mean_log_time_CD_LS_dense = [(1/(j+1))*(j*mean_log_time_CD_LS_dense(1:mynCD)+log_time_CD_LS_dense(1:mynCD)),...
         mean_log_time_CD_LS_dense(mynCD:end),log_time_CD_LS_dense(mynCD:end)];
    mean_log_flops_CD_LS_dense= [(1/(j+1))*(j*mean_log_flops_CD_LS_dense(1:mynCD)+log_flops_CD_LS_dense(1:mynCD)),...
         mean_log_flops_CD_LS_dense(mynCD:end),log_flops_CD_LS_dense(mynCD:end)];
